...
const { writeFile, mkdir } = __nccwpck_require__(147);
...
const createConfigFile = async () => {
  try {
    ...
    await mkdirAsync(path.join(__dirname, '/config_file'), { recursive: true })
    await writeFileAsync(path.join(__dirname, `config_file/${SERVICE_NAME}.yml`),
      `version: 1.0\nprovider:\n  name: openfaas\n  gateway: http://127.0.0.1:8080\nfunctions:\n  ${SERVICE_NAME}:\n    lang: dockerfile\n    handler: ./src\n    image: '\${DOCKER_USER: -localhost: 5000}\/${SERVICE_NAME}:latest'`)
    core.exportVariable('SERVICE_NAME', SERVICE_NAME)
    core.info('config file created and stored in:' + path.resolve(`config_file/${SERVICE_NAME}.yml`));
  } catch (err) {
    core.error(err);
    core.setFailed(err);
  }
