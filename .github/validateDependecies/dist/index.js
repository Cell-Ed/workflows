...
try {
    const fullPath = path.join(core.getInput('GITHUB_WORKSPACE'), 'package.json');
    core.info(`Reading ${fullPath}`);
    const packageJSON = fs.readFileSync(fullPath);
    const pk = JSON.parse(packageJSON);
    const dependencies = { ...pk.dependencies, ...pk.peerDependencies };
    core.info('🔎 Checking dependencies and looking for alpha or beta versions. 🔍');
    const { alpha, beta } = Object.keys(dependencies).reduce((acc, d) => {
        if (dependencies[d].includes('beta')) acc.beta.push(`${d}: ${dependencies[d]}`);
        if (dependencies[d].includes('alpha')) acc.alpha.push(`${d}: ${dependencies[d]}`);
        return acc;
    }, { alpha: [], beta: [] });
    if (!alpha.length && !beta.length) return core.info('✔️ Check completed. ✔️');
    const displayDep = (alpha.length && beta.length)
        ? `Alphas: ${JSON.stringify(alpha)}
        Betas: ${JSON.stringify(beta)}`
        : alpha.length ? `Alphas: ${JSON.stringify(alpha)}` : `Betas: ${JSON.stringify(beta)}`;
    const warningMessage = (alpha.length && beta.length)
        ? 'Alpha and Beta'
        : alpha.length ? 'Alpha' : 'Beta';
    core.info(`🚨 ${warningMessage} dependencies detected! 🚨`)
    throw new Error(`The following dependencies has been detected as alpha or beta version: 
    ${displayDep}
    ⛔ No alpha or beta versions are allowed to be deployed on production. ⛔`);
} catch (err) {
