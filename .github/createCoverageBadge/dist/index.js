...
const generateBadgeColor = percent => {
  ...
  return {
    r: Math.round(red.r + (green.r - red.r) * delta),
    g: Math.round(red.g + (green.g - red.g) * delta),
    b: Math.round(red.b + (green.b - red.b) * delta)
  };
};

const injectHash = async xml => new xml2js.Builder().buildObject({
  svg: {
    ...(await xml2js.parseStringPromise(xml)).svg,
    metadata: {
      contentHash: [uuidV4()]
    }
  }
...
const downloadImage = async () => {
  try {
    const reportPath = process.env.NODE_ENV === 'develop'
      ? 'coverage-summary.json'
      : core.getInput('COVERAGE_REPORT_PATH', { require: true });

    const badgeGeneratorServerUrl = process.env.NODE_ENV === 'develop'
      ? process.env.IMG_SHIELDS_GENERATOR_SERVER_URL || 'https://img.shields.io'
      : core.getInput('BADGE_GENERATOR_SERVER_URL') || 'https://img.shields.io';
    ...
    const [{ data: badgeWithoutLabel }, { data: badgeWithLabel }] = await Promise.all([
    ...
    await Promise.all([
      writeFileAsync(path.join(__dirname, 'badge/coverageBadgeWithoutLabel.svg'), await injectHash(badgeWithoutLabel)),
      badgeWithLabel ? await writeFileAsync(path.join(__dirname, 'badge/coverageBadgeWithLabel.svg'), await injectHash(badgeWithLabel)) : null
    ]);
